...
if (theta_dot > 0) && ((theta>(-theta_lim+sft)) && (theta<(theta_lim+sft)))
    ...
    % begins to stand.
    r = r_av - multiplier*h*tanh(m*(theta-sft));
    dr_dtheta = -(multiplier*h*m)/(cosh(m*(theta-sft))^2);
    dr_dtheta_dot = 0;
elseif (theta_dot < 0) && ((theta>(-theta_lim-sft)) && (theta<(theta_lim-sft)))
